...
while getopts ":cdrtp:" opt; do
  case $opt in
    ...
    t)
      ...
      ;;
    p)
      OPT_TARGET_PATH="$OPTARG"
      ;;
    *)
      echo "This tool builds omim"
      echo "Usage: $0 [-d] [-r] [-c] [-t] [-p TARGET_DIRECTORY]"
      echo
...
{
  CONF=$1
  if [ -n "$OPT_TARGET_PATH" ]; then
    DIRNAME="$OPT_TARGET_PATH/omim-build-$(echo "$CONF" | tr '[:upper:]' '[:lower:]')"
  else
